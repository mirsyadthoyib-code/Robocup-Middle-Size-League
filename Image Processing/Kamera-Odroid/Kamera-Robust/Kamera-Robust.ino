...
void compassLoop(){
  norm = compass.readRaw();
  derajat = atan(norm.XAxis/norm.YAxis)*180/PI;
  
  if (norm.XAxis>0 && norm.YAxis>0){
    //kuadran1
    derajat=derajat;
    }

  else if (norm.XAxis>0 && norm.YAxis<0){
    //kuadran2
    derajat=derajat+180;
    }
